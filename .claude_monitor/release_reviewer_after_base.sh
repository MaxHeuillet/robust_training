...
log "Tracking Base retry chain: 22 job2 IDs + any chained job3 evals. Will release reviewer beta6 jobs (390479-390508) once done."
...
if [ "${#PENDING_SET[@]}" -ne 0 ]; then
  log "WARNING: deadline reached with jobs still unresolved: ${PENDING_SET[*]}. NOT releasing reviewer jobs yet."
  exit 1
fi

log "Releasing all held reviewer beta6 jobs (390479-390508)..."
HELD=$(squeue -u "$USER" -h -t PENDING -o "%i %r" | awk '$1>=390479 && $1<=390508 && $2=="JobHeldUser" {print $1}')
if [ -n "$HELD" ]; then
  scontrol release $HELD
  log "Released $(echo "$HELD" | wc -l) jobs."
else
  log "No held reviewer jobs found (already released or none pending)."
fi
log "Done."
