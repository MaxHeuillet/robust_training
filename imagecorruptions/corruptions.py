...
def disk(radius, alias_blur=0.1, dtype=np.float32):
    ...
    # supersample disk to antialias using skimage.filters.gaussian
...
def clipped_zoom(img, zoom_factor):
    ...
    zoomed = scizoom(img[top0:top0 + ch0, top1:top1 + ch1], (zoom_factor, zoom_factor, 1), order=1)
    return zoomed
...
def gaussian_noise(x, severity=1):
    ...
    x = np.array(x) / 255.
    return np.clip(x + np.random.normal(scale=c, size=x.shape), 0, 1) * 255
...
def speckle_noise(x, severity=1):
    ...
    x = np.array(x) / 255.
    return np.clip(x + x * np.random.normal(scale=c, size=x.shape), 0, 1) * 255
...
def glass_blur(x, severity=1):
    ...
    x = _shuffle_pixels_njit_glass_blur(x.shape[0], x.shape[1], x, c)
...
def defocus_blur(x, severity=1):
    ...
    if len(x.shape) < 3 or x.shape[2] < 3:
        filtered = convolve(x, kernel, mode='reflect')
    else:
        filtered = np.stack([convolve(x[:, :, d], kernel, mode='reflect') for d in range(3)], axis=-1)
    return np.clip(filtered, 0, 1) * 255

def motion_blur(x, severity=1):
    c = [(10, 3), (15, 5), (15, 8), (15, 12), (20, 15)][severity - 1]
    x = np.array(x)
    angle = np.random.uniform(-45, 45)
    return _motion_blur(x, radius=c[0], sigma=c[1], angle=angle)
...
def zoom_blur(x, severity=1):
    ...
    for zoom_factor in c:
        if len(x.shape) < 3 or x.shape[2] < 3:
            x_channels = np.stack([x, x, x], axis=-1)
...
def fog(x, severity=1):
    ...
    x = np.array(x) / 255.
    max_val = x.max()
    fractal = plasma_fractal(mapsize=map_size, wibbledecay=c[1])
    if len(x.shape) < 3 or x.shape[2] < 3:
        x += c[0] * fractal[:shape[0], :shape[1]]
    else:
        x += c[0] * fractal[:shape[0], :shape[1]][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    # Open using PIL and convert to RGB
    frost_img = Image.open(filename).convert("RGB")
    frost_array = np.array(frost_img)
    frost_shape = frost_array.shape
    ...
    if frost_shape[0] >= x_shape[0] and frost_shape[1] >= x_shape[1]:
        ...
        scaling_factor = max(x_shape[0] / frost_shape[0], x_shape[1] / frost_shape[1])
    scaling_factor *= 1.1
    new_shape = (int(np.ceil(frost_shape[1] * scaling_factor)),
                 int(np.ceil(frost_shape[0] * scaling_factor)))
    frost_img_rescaled = frost_img.resize(new_shape, resample=Image.BICUBIC)
    frost_array_rescaled = np.array(frost_img_rescaled)
    x_start = np.random.randint(0, frost_array_rescaled.shape[0] - x_shape[0])
    y_start = np.random.randint(0, frost_array_rescaled.shape[1] - x_shape[1])
    frost_cropped = frost_array_rescaled[x_start:x_start + x_shape[0], y_start:y_start + x_shape[1]]
    if len(x_shape) < 3 or x_shape[2] < 3:
        frost_gray = color.rgb2gray(frost_cropped)
        frost_gray = (frost_gray * 255).astype(np.uint8)
        return np.clip(c[0] * np.array(x) + c[1] * frost_gray, 0, 255)
    else:
        return np.clip(c[0] * np.array(x) + c[1] * frost_cropped, 0, 255)
...
def snow(x, severity=1):
    ...
    if len(x.shape) < 3 or x.shape[2] < 3:
        ...
        x = c[6] * x + (1 - c[6]) * np.maximum(x, x_gray.reshape(x.shape[0], x.shape[1], 1) * 1.5 + 0.5)
    try:
...
def spatter(x, severity=1):
    ...
    # Use PIL to handle the image if not already an Image object.
    x_PIL = x if isinstance(x, Image.Image) else Image.fromarray(np.uint8(x))
    x = np.array(x, dtype=np.float32) / 255.
    ...
    if c[5] == 0:
        ...
        edges = feature.canny(liquid_layer_uint8, sigma=1)
        edges_uint8 = (edges * 255).astype(np.uint8)
        dist = distance_transform_edt(255 - edges_uint8)
        dist = np.minimum(dist, 20)
        dist = uniform_filter(dist, size=3)
        dist = (dist / (np.max(dist) + 1e-8) * 255).astype(np.uint8)
        dist_eq = exposure.equalize_hist(dist)
        dist_eq = (dist_eq * 255).astype(np.uint8)
        ker = np.array([[-2, -1, 0], [-1, 1, 1], [0, 1, 2]])
        dist_filtered = convolve(dist_eq, ker, mode='reflect')
        dist_filtered = uniform_filter(dist_filtered, size=3).astype(np.float32)
        m = np.stack([dist_filtered]*4, axis=-1)
        m /= np.max(m, axis=(0,1), keepdims=True) + 1e-8
        m *= c[4]
        color_layer = np.stack([175/255.*np.ones_like(m[...,0]),
                                238/255.*np.ones_like(m[...,0]),
                                238/255.*np.ones_like(m[...,0])], axis=-1)
        if len(x.shape) < 3 or x.shape[2] < 3:
            add_spatter = np.clip(m[...,0] * color_layer[...,0], 0, 1)
            return np.clip(x + add_spatter, 0, 1) * 255
        else:
            x_bgra = np.concatenate([x, np.ones((*x.shape[:2], 1))], axis=-1)
            spatter_img = np.clip(x_bgra + m[..., :3] * color_layer, 0, 1)
            return spatter_img[..., :3] * 255
    else:
        m = np.where(liquid_layer > c[3], 1, 0)
        m = gaussian(m.astype(np.float32), sigma=c[4])
        m[m < 0.8] = 0
        x_rgb = np.array(x_PIL.convert('RGB'))
        color_layer = np.stack([63/255.*np.ones_like(x_rgb[...,0]),
                                42/255.*np.ones_like(x_rgb[...,0]),
                                20/255.*np.ones_like(x_rgb[...,0])], axis=-1)
        color_layer *= m[..., np.newaxis]
        if len(x.shape) < 3 or x.shape[2] < 3:
            x *= (1 - m)
            return np.clip(x + color.rgb2gray(color_layer), 0, 1) * 255
        else:
            x *= (1 - m[..., np.newaxis])
            return np.clip(x + color_layer, 0, 1) * 255
...
def brightness(x, severity=1):
    ...
    x = np.array(x) / 255.
    if len(x.shape) < 3 or x.shape[2] < 3:
        return np.clip(x + c, 0, 1) * 255
    else:
        x_hsv = color.rgb2hsv(x)
        x_hsv[:, :, 2] = np.clip(x_hsv[:, :, 2] + c, 0, 1)
        return np.clip(color.hsv2rgb(x_hsv), 0, 1) * 255
...
def saturate(x, severity=1):
    ...
    if len(x.shape) < 3 or x.shape[2] < 3:
        ...
        gray_scale = True
    x_hsv = color.rgb2hsv(x)
    x_hsv[:, :, 1] = np.clip(x_hsv[:, :, 1] * c[0] + c[1], 0, 1)
    x_rgb = color.hsv2rgb(x_hsv)
    if gray_scale:
        x_rgb = x_rgb[:, :, 0]
    return np.clip(x_rgb, 0, 1) * 255

def jpeg_compression(x, severity=1):
    c = [25, 18, 15, 10, 7][severity - 1]
    output = BytesIO()
    gray_scale = False
    if not hasattr(x, "save"):
...
def pixelate(x, severity=1):
    ...
    if not hasattr(x, "resize"):
        x = Image.fromarray(np.uint8(x))
    x_shape = np.array(x).shape
    x_resized = x.resize((int(x_shape[1] * c), int(x_shape[0] * c)), resample=Image.BOX)
    x_pixelated = x_resized.resize((x_shape[1], x_shape[0]), resample=Image.NEAREST)
    return x_pixelated
