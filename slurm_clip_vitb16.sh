...
#SBATCH --output=logs/clip_vitb16/%x_%j.out
#SBATCH --error=logs/clip_vitb16/%x_%j.err

source ./execute_setup.sh
set -euo pipefail

THREAT_MODEL="${1:?Usage: sbatch slurm_clip_vitb16.sh <threat_model>}"

SCRIPT_DIR="${SLURM_SUBMIT_DIR:-.}"
SHARD="$SCRIPT_DIR/craft_shard.py"
LOG_DIR="$SCRIPT_DIR/logs/clip_vitb16"
mkdir -p "$LOG_DIR"
...
SURROGATE="clip"
SURROGATE_SLUG="zeroshot_clip_vitb16_laion2b"
...
echo "  Surrogate    : CLIP ViT-B/16 LAION-2B"
